...
public class PreGameController {
    ...
    public static SendingPacket buildTwoUser(ArrayList<Object> objects) throws Exception {
        Object[] objects1 = new Object[1];
        objects1[0] = 1;
        if (!User.getAllUsers().isEmpty()) return new SendingPacket("PreGameMenu","alaki",objects1);;
        Gson gson = new Gson();
        User user1 = gson.fromJson(objects.get(1).toString(), User.class);
        User user2 = gson.fromJson(objects.get(0).toString(), User.class);
        User.getAllUsers().add(user1);
        User.getAllUsers().add(user2);


        return new SendingPacket("PreGameMenu","alaki",objects1);
    }
